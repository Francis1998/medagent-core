"""Tests for the Rivaroxaban + Rifampin Induction Risk safety checker."""

from medagent.models import Medication, RivaroxabanRifampinRisk, Severity
from medagent.safety import RivaroxabanRifampinChecker as ExportedChecker
from medagent.safety.rivaroxaban_rifampin_checker import RivaroxabanRifampinChecker


def _meds(*names: str) -> list[Medication]:
    return [Medication(name=name) for name in names]


def test_no_findings_when_either_class_is_absent() -> None:
    checker = RivaroxabanRifampinChecker()
    assert checker.check(_meds("rivaroxaban dose")) == []
    assert checker.check(_meds("rifampin dose")) == []
    assert checker.check([]) == []


def test_flags_supported_pair_at_expected_severity() -> None:
    checker = RivaroxabanRifampinChecker()
    finding = checker.check(_meds("rivaroxaban dose", "rifampin dose"))[0]
    assert isinstance(finding, RivaroxabanRifampinRisk)
    assert finding.agent == "rivaroxaban"
    assert finding.partner_agent == "rifampin"
    assert finding.severity is Severity.HIGH
    assert "RESEARCH USE ONLY" in finding.rationale


def test_all_supported_agents_participate() -> None:
    checker = RivaroxabanRifampinChecker()
    for primary_agent in ["rivaroxaban", "xarelto"]:
        finding = checker.check(_meds(f"{primary_agent} dose", "rifampin dose"))[0]
        assert finding.agent == primary_agent
    for partner_agent in ["rifampin", "rifampicin", "rimactane"]:
        finding = checker.check(_meds("rivaroxaban dose", f"{partner_agent} dose"))[0]
        assert finding.partner_agent == partner_agent


def test_whole_token_matching_avoids_substring_false_positives() -> None:
    checker = RivaroxabanRifampinChecker()
    assert checker.check(_meds("rivaroxabanlike", "rifampinlike")) == []
    assert len(checker.check(_meds("rivaroxaban", "rifampin"))) == 1


def test_duplicates_are_deduplicated_and_output_is_deterministic() -> None:
    names = ["rifampin dose", "rivaroxaban dose"]
    checker = RivaroxabanRifampinChecker()
    forward = checker.check(_meds(*names))
    reverse = checker.check(_meds(*reversed(names)))
    assert [(i.agent, i.partner_agent) for i in forward] == [
        (i.agent, i.partner_agent) for i in reverse
    ]
    assert (
        len(
            checker.check(
                _meds(
                    "rivaroxaban a",
                    "rivaroxaban b",
                    "rifampin a",
                    "rifampin b",
                )
            )
        )
        == 1
    )


def test_export_alias_matches() -> None:
    assert ExportedChecker is RivaroxabanRifampinChecker


def test_rationale_mentions_research_only() -> None:
    finding = RivaroxabanRifampinChecker().check(_meds("rivaroxaban", "rifampin"))[0]
    assert finding.rationale.startswith("RESEARCH USE ONLY")


def test_severity_ordering_stable() -> None:
    findings = RivaroxabanRifampinChecker().check(_meds("rivaroxaban", "rifampin"))
    assert findings
    ranks = {Severity.HIGH: 3, Severity.CRITICAL: 4}
    assert ranks[findings[0].severity] >= 3
