...
Distinct from related DDI checkers covering other pairs.
"""
...
_PRIMARY_AGENTS: Final[dict[str, str]] = {
    "sofosbuvir": "a direct-acting antiviral nucleotide analog",
    "sovaldi": "a sofosbuvir brand DAA",
    "harvoni": "a ledipasvir/sofosbuvir brand DAA",
    "epclusa": "a sofosbuvir/velpatasvir brand DAA",
}

_PARTNER_AGENTS: Final[dict[str, tuple[str, Severity]]] = {
    "amiodarone": (
        "a class III antiarrhythmic linked to severe bradycardia with sofosbuvir",
        Severity.CRITICAL,
    ),
    "cordarone": (
        "an amiodarone brand antiarrhythmic",
        Severity.CRITICAL,
    ),
    "pacerone": (
        "an amiodarone brand antiarrhythmic",
...
class SofosbuvirAmiodaroneChecker:
    """Flag Sofosbuvir + Amiodarone Bradycardia Risk pairs."""

    def check(self, medications: list[Medication]) -> list[SofosbuvirAmiodaroneRisk]:
        ...
        if not primary_matches or not partner_matches:
            logger.info("sofosbuvir_amiodarone_checker_checked", findings=0)
            return []
        ...
        findings: list[SofosbuvirAmiodaroneRisk] = []
        ...
        logger.info("sofosbuvir_amiodarone_checker_checked", findings=len(findings))
        return findings
    ...
    def _build_rationale(
        *,
        ...
        return (
            ...
            "Coadministration of sofosbuvir-containing HCV regimens with amiodarone "
            "can cause serious symptomatic bradycardia. "
