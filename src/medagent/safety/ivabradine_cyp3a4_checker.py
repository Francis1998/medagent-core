"""Ivabradine + Strong CYP3A4 Inhibitor Bradycardia safety checker.
...
_PRIMARY_AGENTS: Final[dict[str, str]] = {
    "ivabradine": ("a funny-current (If) inhibitor metabolized by CYP3A4"),
    "corlanor": ("an ivabradine brand If inhibitor"),
}
...
class IvabradineCyp3a4Checker:
    """Flag ivabradine CYP3A4 co-prescription risks."""

    def check(self, medications: list[Medication]) -> list[IvabradineCyp3a4Risk]:
        ...
        if not primary_matches or not partner_matches:
            logger.info("ivabradine_cyp3a4_checked", findings=0)
            return []
        ...
        findings: list[IvabradineCyp3a4Risk] = []
        ...
        logger.info("ivabradine_cyp3a4_checked", findings=len(findings))
        return findings
