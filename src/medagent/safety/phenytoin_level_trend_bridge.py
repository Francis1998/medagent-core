"""Phenytoin + serial serum phenytoin level toxicity-trend bridge.

Existing PhenytoinFluconazoleChecker covers CYP2C9 DDI pairs without serial level trends.

This bridge fills that gap: it combines phenytoin exposure with rising,
elevated, or clearly supratherapeutic serial phenytoin levels into advisory
:class:`~medagent.models.PhenytoinLevelTrendAlert` findings.
...
from medagent.models import Medication, PhenytoinLevelTrendAlert, Severity
...
# Advisory thresholds (mcg/mL) — RESEARCH USE ONLY, not dosing guidance.
_ELEVATED_LEVEL: Final[float] = 20.0
_SUPRATHERAPEUTIC_LEVEL: Final[float] = 30.0
_MIN_RELATIVE_RISE: Final[float] = 0.25


class PhenytoinLevelTrendBridge:
    """Map phenytoin exposure + serial serum levels to toxicity cues."""

    def check(
        self,
        medications: list[Medication],
        labs: Sequence[Mapping[str, Any]] | None = None,
    ) -> list[PhenytoinLevelTrendAlert]:
        """Return phenytoin + serum-level-trend toxicity advisories.
        ...
        Returns:
            Zero or more :class:`PhenytoinLevelTrendAlert` findings. Never modifies medications.
        """
        ...
        for entry in labs or []:
            ...
            looks_like_level = "phenytoin" in name and (
        ...
        if not agents:
            logger.info("phenytoin_level_trend_bridge_checked", findings=0)
            return []

        findings: list[PhenytoinLevelTrendAlert] = []

        def _add(kind: str, severity: Severity, rationale: str) -> None:
            findings.append(
                PhenytoinLevelTrendAlert(
                    finding_kind=kind,
                    agents=agents,
        ...
        if (significant_rise or (rising and elevated)) and agents:
            ...
            _add(
                ...
                sev,
                (
                    "RESEARCH USE ONLY: Rising serum phenytoin level trend "
                    f"(values {values}"
                    + (f", {percent:.1f}%" if percent is not None else "")
                    + f"). Agents: {', '.join(agents)}. Phenytoin-level bridge "
        ...
        if elevated and agents:
            sev = Severity.CRITICAL if critical else Severity.HIGH
            kind = "supratherapeutic_phenytoin_level" if critical else "elevated_phenytoin_level"
            _add(
                kind,
                sev,
                (
                    ...
                    + f" serum phenytoin level (latest {latest}, series {values}). "
        ...
        if (significant_rise or elevated) and agents:
            _add(
                ...
                (
                    "RESEARCH USE ONLY: Phenytoin serum-level monitoring advisory "
        ...
        logger.info("phenytoin_level_trend_bridge_checked", findings=len(findings))
        return findings
