"""Tacrolimus + Strong CYP3A4 Inhibitor Exposure safety checker.
...
_PRIMARY_AGENTS: Final[dict[str, str]] = {
    "tacrolimus": ("a calcineurin inhibitor immunosuppressant metabolized by CYP3A4"),
...
_PARTNER_AGENTS: Final[dict[str, tuple[str, Severity]]] = {
    ...
    "cobicistat": (
        "a strong CYP3A4 pharmacokinetic booster",
        Severity.CRITICAL,
    ),
}
...
class TacrolimusCyp3a4Checker:
    """Flag tacrolimus CYP3A4 co-prescription risks."""

    def check(self, medications: list[Medication]) -> list[TacrolimusCyp3a4Risk]:
        ...
        if not primary_matches or not partner_matches:
            logger.info("tacrolimus_cyp3a4_checked", findings=0)
            return []
        ...
        findings: list[TacrolimusCyp3a4Risk] = []
        ...
        logger.info("tacrolimus_cyp3a4_checked", findings=len(findings))
        return findings
    ...
    def _build_rationale(
        *,
        ...
        return (
            ...
            "Strong CYP3A4 inhibition can markedly elevate tacrolimus "
            "exposure and intensify nephrotoxicity / neurotoxicity risk. "
