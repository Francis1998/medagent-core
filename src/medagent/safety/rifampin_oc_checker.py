...
alternative or additional contraception when co-prescribed.  Distinct from
other CYP3A4 interaction checkers.
"""
...
_PRIMARY_AGENTS: Final[dict[str, str]] = {
    "rifampin": "a potent CYP3A4-inducing antimycobacterial",
    "rifampicin": "a potent CYP3A4-inducing antimycobacterial",
    "rifadin": "a rifampin brand antimycobacterial",
    "rimactane": "a rifampin brand antimycobacterial",
}

_PARTNER_AGENTS: Final[dict[str, tuple[str, Severity]]] = {
    "ethinylestradiol": (
        "an oral contraceptive estrogen component",
        Severity.HIGH,
    ),
    "levonorgestrel": (
        "a progestin-only or combined oral contraceptive",
        Severity.HIGH,
    ),
    "norethindrone": (
        "a progestin oral contraceptive component",
        Severity.HIGH,
    ),
    "desogestrel": (
        "a progestin oral contraceptive component",
...
class RifampinOcChecker:
    """Flag Rifampin + Oral Contraceptive pairs."""

    def check(self, medications: list[Medication]) -> list[RifampinOcRisk]:
        ...
        if not primary_matches or not partner_matches:
            logger.info("rifampin_oc_checker_checked", findings=0)
            return []
        ...
        logger.info("rifampin_oc_checker_checked", findings=len(findings))
        return findings
    ...
    def _build_rationale(
        *,
        ...
        return (
            ...
            "Rifampin is a potent CYP3A4 inducer that can dramatically reduce "
            "oral contraceptive efficacy, creating contraceptive failure risk. "
            "Alternative or additional contraception should be recommended."
