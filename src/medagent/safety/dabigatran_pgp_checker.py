"""Dabigatran + Strong P-gp Inhibitor Bleed Risk safety checker.
...
_PRIMARY_AGENTS: Final[dict[str, str]] = {
    "dabigatran": ("a direct thrombin inhibitor DOAC that is a P-gp substrate"),
    "pradaxa": ("a dabigatran brand direct thrombin inhibitor"),
}

_PARTNER_AGENTS: Final[dict[str, tuple[str, Severity]]] = {
    "dronedarone": (
        "a strong P-gp inhibitor antiarrhythmic",
        Severity.CRITICAL,
    ),
    "ketoconazole": (
        "a strong P-gp / CYP3A4-inhibiting azole antifungal",
        Severity.CRITICAL,
    ),
    "itraconazole": (
        "a strong P-gp / CYP3A4-inhibiting azole antifungal",
    ...
    "cyclosporine": (
        "a strong P-gp inhibitor calcineurin inhibitor",
        Severity.CRITICAL,
    ),
    "ciclosporin": (
        "a cyclosporine spelling variant and strong P-gp inhibitor",
        Severity.CRITICAL,
    ),
}
...
class DabigatranPgpChecker:
    """Flag dabigatran P-gp co-prescription risks."""

    def check(self, medications: list[Medication]) -> list[DabigatranPgpRisk]:
        ...
        if not primary_matches or not partner_matches:
            logger.info("dabigatran_pgp_checked", findings=0)
            return []
        ...
        findings: list[DabigatranPgpRisk] = []
        ...
        logger.info("dabigatran_pgp_checked", findings=len(findings))
        return findings
