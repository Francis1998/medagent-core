"""Warfarin + TMP-SMX INR elevation / bleed-risk safety checker.

TMP-SMX can potentiate warfarin anticoagulation, elevate INR, and increase
major bleeding risk. This warfarin × TMP-SMX control is distinct from
methotrexate + TMP-SMX and fluoroquinolone + warfarin checkers.
"""
...
_PRIMARY_AGENTS: Final[dict[str, str]] = {
    "warfarin": "a vitamin K antagonist anticoagulant with a narrow therapeutic index",
    "coumadin": "a warfarin brand vitamin K antagonist anticoagulant",
    "jantoven": "a warfarin brand vitamin K antagonist anticoagulant",
}

_PARTNER_AGENTS: Final[dict[str, tuple[str, Severity]]] = {
    "trimethoprim": (
        "a dihydrofolate-reductase inhibitor often combined as TMP-SMX",
        Severity.CRITICAL,
    ),
    "sulfamethoxazole": (
        "a sulfonamide antibiotic often combined as TMP-SMX",
        Severity.CRITICAL,
    ),
    "bactrim": (
...
class WarfarinTmpsmxChecker:
    """Flag warfarin co-prescribed with TMP-SMX."""

    def check(self, medications: list[Medication]) -> list[WarfarinTmpsmxRisk]:
        """Return one finding per unique warfarin × TMP-SMX pair."""
        ...
        if not primary_matches or not partner_matches:
            logger.info("warfarin_tmpsmx_checked", findings=0)
            return []
        ...
        findings: list[WarfarinTmpsmxRisk] = []
        ...
        logger.info("warfarin_tmpsmx_checked", findings=len(findings))
        return findings
    ...
    def _build_rationale(
        *,
        ...
        return (
            ...
            "TMP-SMX can potentiate warfarin anticoagulation, elevate INR, and "
            "increase major bleeding risk. Obtain urgent qualified clinical and "
