"""Valproate + carbapenem precipitous level-drop safety checker.

Carbapenems can cause a rapid, clinically significant drop in serum valproate
levels, precipitating breakthrough seizures. This focused valproate ×
carbapenem control is distinct from general AED interaction screens.
"""
...
_PRIMARY_AGENTS: Final[dict[str, str]] = {
    "valproate": "an antiepileptic with a narrow therapeutic index",
    "valproic acid": "an antiepileptic with a narrow therapeutic index",
    "divalproex": "a valproate salt formulation and antiepileptic",
    "depakote": "a divalproex brand formulation and antiepileptic",
    "depakene": "a valproic acid brand formulation and antiepileptic",
}

_PARTNER_AGENTS: Final[dict[str, tuple[str, Severity]]] = {
    "meropenem": (
        "a carbapenem antibiotic that can precipitously lower valproate levels",
        Severity.CRITICAL,
    ),
    "ertapenem": (
        "a carbapenem antibiotic that can precipitously lower valproate levels",
    ...
    "carbapenem": (
        "a carbapenem-class antibiotic that can precipitously lower valproate levels",
        Severity.CRITICAL,
    ),
}
...
class ValproateCarbapenemChecker:
    """Flag valproate therapy co-prescribed with carbapenem antibiotics."""

    def check(self, medications: list[Medication]) -> list[ValproateCarbapenemRisk]:
        """Return one finding per unique valproate × carbapenem pair."""
        ...
        if not primary_matches or not partner_matches:
            logger.info("valproate_carbapenem_checked", findings=0)
            return []
        ...
        findings: list[ValproateCarbapenemRisk] = []
        ...
        logger.info("valproate_carbapenem_checked", findings=len(findings))
        return findings
    ...
    def _build_rationale(
        *,
        ...
        return (
            ...
            "Carbapenems can cause a precipitous drop in serum valproate "
            "levels within days, increasing breakthrough seizure risk. "
            "Promptly review therapy with a qualified clinician; do not "
