...
Amlodipine is a CYP3A4 substrate; clarithromycin is a strong CYP3A4 inhibitor. Co-prescription
is linked to hospitalization for hypotension/shock in older adults. Distinct from
simvastatin–macrolide and generic CCB panels.
"""
...
_PRIMARY_AGENTS: Final[dict[str, str]] = {
    ...
    "norvasc": "an amlodipine brand dihydropyridine calcium-channel blocker",
}
...
_PARTNER_AGENTS: Final[dict[str, tuple[str, Severity]]] = {
    ...
    "biaxin": (
        "a clarithromycin brand strong CYP3A4-inhibiting macrolide",
        Severity.CRITICAL,
    ),
}
...
class AmlodipineClarithromycinChecker:
    """Flag Amlodipine + Clarithromycin pairs."""

    def check(self, medications: list[Medication]) -> list[AmlodipineClarithromycinRisk]:
        ...
        if not primary_matches or not partner_matches:
            logger.info("amlodipine_clarithromycin_checker_checked", findings=0)
            return []
        ...
        findings: list[AmlodipineClarithromycinRisk] = []
        ...
        logger.info("amlodipine_clarithromycin_checker_checked", findings=len(findings))
        return findings
