...
Distinct from lamotrigine + valproate SJS/TEN checking and rifampin + OC screens.
"""
...
_PRIMARY_AGENTS: Final[dict[str, str]] = {
    "lamotrigine": "an antiepileptic UGT substrate with a narrow therapeutic window",
    "lamictal": "a lamotrigine brand antiepileptic",
}

_PARTNER_AGENTS: Final[dict[str, tuple[str, Severity]]] = {
    "ethinylestradiol": (
        "an estrogen component of combined oral contraceptives that induces UGT",
        Severity.HIGH,
    ),
    "ethinyloestradiol": (
        "a British spelling ethinylestradiol estrogen that induces UGT",
        Severity.HIGH,
    ),
    "ethinyl": (
        "an ethinylestradiol token cue in combined oral contraceptive names",
        Severity.HIGH,
    ),
    "yaz": (
        "an ethinylestradiol-containing combined oral contraceptive brand",
        Severity.HIGH,
    ),
    "yasmin": (
        "an ethinylestradiol-containing combined oral contraceptive brand",
...
class EthinylestradiolLamotrigineChecker:
    """Flag Ethinylestradiol + Lamotrigine Level Reduction Risk pairs."""

    def check(self, medications: list[Medication]) -> list[EthinylestradiolLamotrigineRisk]:
        ...
        if not primary_matches or not partner_matches:
            logger.info("ethinylestradiol_lamotrigine_checker_checked", findings=0)
            return []
        ...
        findings: list[EthinylestradiolLamotrigineRisk] = []
        ...
        for primary_index, primary_med, primary_agent in primary_matches:
            for partner_index, partner_med, partner_agent in partner_matches:
                ...
                findings.append(
                    EthinylestradiolLamotrigineRisk(
        ...
        logger.info("ethinylestradiol_lamotrigine_checker_checked", findings=len(findings))
        return findings
