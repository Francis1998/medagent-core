"""Methotrexate + trimethoprim / TMP-SMX antifolate synergy safety checker.

Trimethoprim and TMP-SMX intensify antifolate toxicity with methotrexate and
can precipitate pancytopenia. This brand-aware methotrexate × trimethoprim
control is distinct from the legacy `mtx_tmpsmx_checker` and warfarin +
TMP-SMX INR checkers.
"""
...
_PARTNER_AGENTS: Final[dict[str, tuple[str, Severity]]] = {
    "trimethoprim": (
        "a dihydrofolate-reductase inhibitor that intensifies antifolate toxicity",
        Severity.CRITICAL,
    ),
    "tmp-smx": (
...
class MethotrexateTrimethoprimChecker:
    """Flag methotrexate co-prescribed with trimethoprim / TMP-SMX."""

    def check(self, medications: list[Medication]) -> list[MethotrexateTrimethoprimRisk]:
        """Return one finding per unique methotrexate × trimethoprim pair."""
        ...
        if not primary_matches or not partner_matches:
            logger.info("methotrexate_trimethoprim_checked", findings=0)
            return []
        ...
        findings: list[MethotrexateTrimethoprimRisk] = []
        ...
        logger.info("methotrexate_trimethoprim_checked", findings=len(findings))
        return findings
    ...
    def _build_rationale(
        *,
        ...
        return (
            ...
            "Trimethoprim / TMP-SMX intensifies antifolate synergy with "
            "methotrexate and can precipitate pancytopenia. Obtain urgent "
