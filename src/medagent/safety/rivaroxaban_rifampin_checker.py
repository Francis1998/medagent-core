...
_PRIMARY_AGENTS: Final[dict[str, str]] = {
    "rivaroxaban": "a factor Xa inhibitor DOAC CYP3A4/P-gp substrate",
    "xarelto": "a rivaroxaban brand DOAC",
}

_PARTNER_AGENTS: Final[dict[str, tuple[str, Severity]]] = {
    "rifampin": (
        "a strong CYP3A4/P-gp inducer that reduces DOAC exposure",
        Severity.HIGH,
    ),
    "rifampicin": (
        "a rifampin synonym strong inducer",
        Severity.HIGH,
    ),
    "rimactane": (
        "a rifampin brand strong inducer",
        Severity.HIGH,
    ),
}
...
class RivaroxabanRifampinChecker:
    """Flag Rivaroxaban + Rifampin Induction Risk pairs."""

    def check(self, medications: list[Medication]) -> list[RivaroxabanRifampinRisk]:
        ...
        if not primary_matches or not partner_matches:
            logger.info("rivaroxaban_rifampin_checker_checked", findings=0)
            return []
        ...
        findings: list[RivaroxabanRifampinRisk] = []
        ...
        logger.info("rivaroxaban_rifampin_checker_checked", findings=len(findings))
        return findings
    ...
    def _build_rationale(
        *,
        ...
        return (
            ...
            "Rifampin strongly induces CYP3A4 and P-gp, reducing rivaroxaban "
            "exposure and increasing thrombotic risk. "
