"""Cyclosporine + serial serum cyclosporine level toxicity-trend bridge.

Existing CyclosporineStatinChecker covers statin myopathy DDI pairs without serial trough trends.

This bridge fills that gap: it combines cyclosporine exposure with rising,
elevated, or clearly supratherapeutic serial cyclosporine levels into advisory
:class:`~medagent.models.CyclosporineLevelTrendAlert` findings.
...
from medagent.models import CyclosporineLevelTrendAlert, Medication, Severity
...
_ELEVATED_LEVEL: Final[float] = 400.0
_SUPRATHERAPEUTIC_LEVEL: Final[float] = 500.0
_MIN_RELATIVE_RISE: Final[float] = 0.25


class CyclosporineLevelTrendBridge:
    ...
    def check(
    ...
    ) -> list[CyclosporineLevelTrendAlert]:
        """Return cyclosporine + serum-level-trend toxicity advisories.
        ...
        Returns:
            Zero or more :class:`CyclosporineLevelTrendAlert` findings. Never modifies medications.
        """
        ...
        for entry in labs or []:
            ...
            looks_like_level = "cyclosporine" in name and (
        ...
        if not agents:
            logger.info("cyclosporine_level_trend_bridge_checked", findings=0)
            return []

        findings: list[CyclosporineLevelTrendAlert] = []

        def _add(kind: str, severity: Severity, rationale: str) -> None:
            findings.append(
                CyclosporineLevelTrendAlert(
                    finding_kind=kind,
                    agents=agents,
        ...
        if (significant_rise or (rising and elevated)) and agents:
            ...
            _add(
                ...
                sev,
                (
                    "RESEARCH USE ONLY: Rising serum cyclosporine level trend "
                    f"(values {values}"
                    + (f", {percent:.1f}%" if percent is not None else "")
                    + f"). Agents: {', '.join(agents)}. Cyclosporine-level bridge "
        ...
        if elevated and agents:
            sev = Severity.CRITICAL if critical else Severity.HIGH
            kind = (
                "supratherapeutic_cyclosporine_level" if critical else "elevated_cyclosporine_level"
            )
            _add(
                kind,
                sev,
                (
                    ...
                    + f" serum cyclosporine level (latest {latest}, series {values}). "
        ...
        if (significant_rise or elevated) and agents:
            _add(
                ...
                (
                    "RESEARCH USE ONLY: Cyclosporine serum-level monitoring advisory "
        ...
        logger.info("cyclosporine_level_trend_bridge_checked", findings=len(findings))
        return findings
