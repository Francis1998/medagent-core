"""Simvastatin + Amiodarone Myopathy/Rhabdomyolysis safety checker.
...
_PRIMARY_AGENTS: Final[dict[str, str]] = {
    "simvastatin": "an HMG-CoA reductase inhibitor metabolized by CYP3A4",
    "zocor": "a simvastatin brand HMG-CoA reductase inhibitor",
}

_PARTNER_AGENTS: Final[dict[str, tuple[str, Severity]]] = {
    "amiodarone": (
        "a class III antiarrhythmic that inhibits CYP3A4",
        Severity.CRITICAL,
    ),
    "cordarone": (
        "an amiodarone brand formulation that inhibits CYP3A4",
        Severity.CRITICAL,
    ),
}
...
class SimvastatinAmiodaroneChecker:
    """Flag simvastatin amiodarone co-prescription risks."""

    def check(self, medications: list[Medication]) -> list[SimvastatinAmiodaroneRisk]:
        ...
        if not primary_matches or not partner_matches:
            logger.info("simvastatin_amiodarone_checked", findings=0)
            return []
        ...
        findings: list[SimvastatinAmiodaroneRisk] = []
        ...
        logger.info("simvastatin_amiodarone_checked", findings=len(findings))
        return findings
    ...
    def _build_rationale(
        *,
        ...
        return (
            ...
            "Amiodarone inhibits CYP3A4-mediated simvastatin metabolism, "
            "increasing statin exposure and myopathy/rhabdomyolysis risk "
            "(FDA dose-limit warning). Obtain urgent qualified clinical and "
