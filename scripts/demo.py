...
DELAY = 0.4  # seconds between animation steps (overridden by --no-delay)
    ...
    "chest_pain_mi": {
        ...
        "patient": {
            ...
            "clinical_notes": (
                ...
                "to left arm. Associated diaphoresis and nausea. Hx: hypertension, T2DM. "
                "ECG: ST elevation II, III, aVF. Troponin-I 2.4 ng/mL (ref <0.04)."
            ),
        ...
        "docs": [
            ...
            ("LocalKB", "kb_004", "Myocardial Infarction — Biomarkers (Troponin I and T)", 0.85),
            ("LocalKB", "kb_001", "ST Elevation MI — ECG criteria and management pathway", 0.81),
        ],
        "interaction": ("aspirin", "metoprolol", "MODERATE", "Additive bradycardia risk", ["rxnorm", "openfda"]),
        "hypotheses": [
        ...
        "next_steps": [
            ...
            "Repeat ECG every 30 minutes or with symptom changes",
            "Monitor for aspirin + metoprolol interaction — bradycardia risk MODERATE",
        ],
    },
    "drug_interaction": {
        "title": "Polypharmacy — High-Risk Drug Combination",
        "patient": {
            "age": 78,
            "sex": "female",
            "chief_complaint": "Medication reconciliation for new warfarin + amiodarone co-prescription",
            "clinical_notes": (
        ...
        "docs": [
            ...
            ("OpenFDA", "warfarin-label", "FDA warfarin label: amiodarone interaction section", 0.91),
            ("LocalKB", "kb_002", "Warfarin Drug Interactions — CYP2C9/2C19 pathways", 0.88),
        ],
        "interactions": [
            ("warfarin", "amiodarone", "CRITICAL", "CYP2C9 inhibition → 3-5x INR elevation → bleeding", ["rxnorm", "openfda"]),
            ("warfarin", "aspirin", "MODERATE", "Additive anticoagulation + GI mucosal damage", ["rxnorm", "openfda"]),
            ("omeprazole", "warfarin", "MODERATE", "CYP2C19 inhibition modestly increases warfarin AUC", ["rxnorm", "openfda"]),
        ],
        "confidence": 0.72,
        "escalated": False,
    },
    "escalate": {
        "title": "Ambiguous Presentation — ESCALATE Triggered",
        "patient": {
            "age": 42,
            "sex": "female",
            ...
            "clinical_notes": (
                ...
                "and 8kg unintentional weight loss. No localising symptoms. No known "
                "exposures. Family history non-contributory."
            ),
            "medications": ["none"],
            "labs": {"Hemoglobin": "11.2 g/dL [LOW]", "LDH": "Not available", "HIV": "Not tested"},
        },
        "entities": [
            ("fatigue", "DISEASE"),
        ...
        "docs": [
            ...
            ("PubMed", "32145678", "Constitutional symptoms: differential diagnosis approach", 0.71),
        ],
        "hypotheses": [
        ...
        "uncertainty_flags": [
            ...
            "Strong FOR and AGAINST evidence on top hypothesis",
...
def sleep(seconds: float) -> None:
    """Sleep only if DELAY is non-zero (skipped in --no-delay mode)."""
    if DELAY > 0:
        time.sleep(seconds * DELAY / 0.4)


def state_badge(state: str, color: str = "blue") -> Text:
    """Render a bold coloured state badge."""
    return Text(f" {state} ", style=f"bold {color} on {color}")
...
def render_header(case_title: str) -> None:
    ...
    banner.append("  ██║ ╚═╝ ██║███████╗██████╔╝██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║   \n", style="bold magenta")
    banner.append("  ╚═╝     ╚═╝╚══════╝╚═════╝ ╚═╝  ╚═╝ ╚═════╝ ╚══════╝╚═╝  ╚═══╝   ╚═╝   \n", style="bold magenta")
    banner.append("                     C O R E  —  clinical reasoning agent\n", style="dim italic")
...
def render_extraction(entities: list[tuple[str, str]]) -> None:
    ...
    label_colors = {
        "DISEASE": "red", "CHEMICAL": "yellow", "GENE": "cyan",
        "PROTEIN": "magenta", "ORGANISM": "blue",
    }
...
def render_retrieval(docs: list[tuple[str, str, str, float]], interaction: tuple | None = None) -> None:
...
def render_reasoning(hypotheses: list[tuple[str, str, float, list, list, str | None]]) -> None:
    """Display REASONING results with Bayesian scores."""
    console.print()
    console.print(Rule("[bold blue]◆ REASONING[/]"))
    sleep(0.6)

    with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}"), TimeElapsedColumn(), transient=True) as p:
...
def render_safety_check(confidence: float, escalated: bool, flags: list[str] | None = None) -> None:
    """Display SAFETY_CHECK outcome."""
    console.print()
...
def render_output(
    ...
    """Display the final ClinicalReasoning output."""
    console.print()
    console.print(Rule("[bold green]◆ OUTPUT[/]" if not case["escalated"] else "[bold red]◆ ESCALATED OUTPUT[/]"))
    sleep(0.5)
    ...
    console.print(
        Panel(
            "[bold yellow]⚠  RESEARCH USE ONLY[/]  This output has NOT been reviewed by a licensed "
            "clinician. It is NOT FDA-cleared and MUST NOT be used to guide clinical treatment "
...
def run_chest_pain(case: dict[str, Any]) -> None:
    """Run the chest pain / STEMI demo case."""
...
def run_drug_interaction(case: dict[str, Any]) -> None:
    """Run the polypharmacy / drug interaction demo case."""
    render_header(case["title"])
    render_intake(case["patient"])
    render_extraction(case["entities"])
...
def run_escalate(case: dict[str, Any]) -> None:
    """Run the ambiguous presentation / ESCALATE demo case."""
    render_header(case["title"])
    render_intake(case["patient"])
    render_extraction(case["entities"])
...
async def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Clinical case to demonstrate",
    )
    parser.add_argument(
        "--no-delay",
        action="store_true",
        help="Skip animation delays (useful for CI screenshots)",
    )
